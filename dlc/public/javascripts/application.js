(function($) {
  $(document).ready(function(){
    $('.baseline-travel input').change(function(e){
      ...
      if (alone) {
        $('.number', $baseline).text(((green/alone)*100+'').substr(0,5));
        $('.line-percent',$baseline).show();
      } else {
        $('.line-percent',$baseline).hide();
      }
      var $form  = $baseline.parents('.baseline');
      var t_miles = _.reduce($('.baseline-travel input.alone', $form).get(), 0, function(t,i){return t+parseFloat($(i).val())});
      ...
      if (t_miles) {
        $('.foot .number', $form).text(((g_miles/t_miles)*100+'').substr(0,5));
